...
`ifdef	FORMAL
		parameter  [(TIMER_BITS-1):0]	CLOCKS_PER_BAUD = 16, `else
		parameter  [(TIMER_BITS-1):0]	CLOCKS_PER_BAUD = 868,	`endif
...
`endif
	...
	always @(posedge i_clk)
		cover(o_wr); always @(posedge i_clk)
	begin
		cover(!ck_uart);
		cover((f_past_valid)&&($rose(ck_uart)));               cover((zero_baud_counter)&&(state == RXUL_BIT_ZERO)); cover((zero_baud_counter)&&(state == RXUL_BIT_ONE));  cover((zero_baud_counter)&&(state == RXUL_BIT_TWO));  cover((zero_baud_counter)&&(state == RXUL_BIT_THREE));cover((zero_baud_counter)&&(state == RXUL_BIT_FOUR)); cover((zero_baud_counter)&&(state == RXUL_BIT_FIVE)); cover((zero_baud_counter)&&(state == RXUL_BIT_SIX));  cover((zero_baud_counter)&&(state == RXUL_BIT_SEVEN));cover((zero_baud_counter)&&(state == RXUL_STOP));     cover((zero_baud_counter)&&(state == RXUL_WAIT));     end
`endif
