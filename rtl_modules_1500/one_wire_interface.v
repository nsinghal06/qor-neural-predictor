...
module one_wire_interface (
   ...
   input        pdr;
   input        OW_LOW;          input        OW_SHORT;        input        rbf;             input [7:0]  rcvr_buffer;
   input        RD_bar;
   input        reset_owr;
   input        rsrf;            input        temt;
   input        WR_bar;

   output       BIT_CTL;         output       CLK_EN;          output       clr_activate_intr;
   output       DDIR;
   output       div_1;           output       div_2;           output       div_3;           output [7:0] DOUT;
   output       EN_FOW;          output       EOWL;            output       EOWSH;           output       epd;             output       erbf;            output       ersf;            output       etbe;            output       etmt;            output       FOW;             output       ias;             output       LLM;             output       OD;              output       owr;
   output       pd;
   output       PPM;             output       pre_0;           output       pre_1;           output       rbf_reset;       output       sr_a;
   output       STP_SPLY;        output       STPEN;           output       tbe;
   ...
   wire         read_op;
   wire         write_op;
   reg [2:0]    sel_addr;         reg       sr_a;                reg       owr;                 reg       FOW;                 wire [7:0] CMD_REG = {4'b0, DQ_IN, FOW, sr_a, owr};

   reg       set_activate_intr;
   reg       clr_activate_intr;
   reg  xmit_buffer_full;

   reg [7:0] xmit_buffer;         reg       OD;                  reg       BIT_CTL;             reg       STP_SPLY;            reg       STPEN;               reg       EN_FOW;              reg       PPM;                 reg       LLM;                 wire [7:0] CONTRL_REG = {1'b0, OD, BIT_CTL, STP_SPLY, STPEN, EN_FOW,
             PPM, LLM};

   wire      OW_LOW;              wire      OW_SHORT;            reg       pd;                  wire      pdr;                 reg       tbe;                 wire      rbf;                 wire [7:0] INT_REG = {OW_LOW, OW_SHORT, rsrf, rbf, temt, tbe, pdr, pd};

   reg       EOWL;                reg       EOWSH;               reg       epd;                 reg       ias;                 reg       etbe;                reg       etmt;                reg       erbf;                reg       ersf;                wire [7:0] INTEN_REG = {EOWL, EOWSH, ersf, erbf, etmt, etbe, ias, epd};

   reg       pre_0;               reg       pre_1;               reg       div_1;               reg       div_2;               reg       div_3;               reg       CLK_EN;              wire [7:0] CLKDV_REG = {CLK_EN, 2'b0, div_3, div_2, div_1, pre_1, pre_0};
  ...
  wire acint_reset = MR || (clear_interrupts); always @(posedge acint_reset or posedge RD_bar)
    if(acint_reset)
  ...
  always @(posedge MR or posedge FSM_CLK)
    if (MR)
      ...
      pd <= 1'b1;
    else if (clr_activate_intr)  pd <= 1'b0;
    else
      pd <= pd;
