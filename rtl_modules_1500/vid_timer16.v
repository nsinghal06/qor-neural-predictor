...
always @(posedge clk) begin
   if(rst_n==0) begin
      ...
      if(timer==FULL_TIMER || timer==HALF_TIMER) begin
        sf_cnt <= (sf_cnt==1049) ? 11'h0 : sf_cnt+11'h1;  end
   end
end
...
always @(posedge clk) begin
   if(rst_n==0) begin
      ...
      case( state) 
        5'h0:  begin  hs <= 0;
                 if(sf_cnt== 6) state <= 5'h1;  end
        5'h1:  begin  hs <= 1;
                 if(sf_cnt== 12) state <= 5'h2;  end
        5'h2:  begin  hs <= 0;
                 if(sf_cnt== 18) state <= 5'h3;  end
        5'h3:  begin  hs <= 0;
                 if(sf_cnt== 525) state <= 5'h4;  end
        5'h4:  begin  hs <= 0;
                 if(sf_cnt== 531) state <= 5'h5;  end
        5'h5:  begin  hs <= 1;
                 if(sf_cnt== 537) state <= 5'h6;  end
        5'h6:  begin  hs <= 0;
                 if(sf_cnt== 542) state <= 5'h7;  end
        5'h7:  begin  hs <= 0;
                 if(sf_cnt== 1049) state <= 5'h0;  end
      endcase
   end
end
